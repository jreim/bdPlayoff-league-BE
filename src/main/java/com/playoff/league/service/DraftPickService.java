...
public class DraftPickService {
	...
	private static final Logger log = LoggerFactory.getLogger(DraftPickService.class);
	public void persistPick(DraftPick pick){
      ObjectifyService.ofy().save().entities(pick).now();
	}

	public boolean isDraftStarted(){
		return !this.getAllPicks().isEmpty();
	}
